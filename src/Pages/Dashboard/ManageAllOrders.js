                            ...
                            orders.map((order, index) =>
                                <tr key={order._id}>
                                    ...
                                    <td>
                                        {
                                            !order.paid &&
                                            < label onClick={() => setDeleteOrders(order)} htmlFor="delete-confirm-modal" title="Delete the order">
                                                <svg xmlns="http://www.w3.org/2000/svg" className="btn btn-outline rounded-full" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                            </label>
                                        }
                                        {
                                            order.paid && !order.status && <>

                                                <label onClick={() => shippedOrder(order._id)} title="Shipped the order" className="btn btn-outline rounded-full"> Shipped
                                                </label>
                                            </>
                                        }
                                        {
